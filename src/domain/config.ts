...
export interface TradingConfig {
  ...
  /**
   * Estimated time (ms) from deciding to trade until both legs are actually
   * filled — order build, network round-trips, matching. Added to the slowest
   * leg's book age to get the total window during which the market can move
   * against us. The latency-risk model rejects edges that can't survive it.
   */
  executionLatencyMs: number;

  /**
   * Fallback BTC volatility as a fraction of price per second, used by the
   * latency-risk model before enough live ticks have been observed to measure
   * it directly. 0.0006 ≈ 0.06%/s is a conservative crypto intraday estimate.
   */
  volatilityPctPerSec: number;

  /**
   * Confidence multiplier on the estimated adverse move (how many "expected
   * moves" we budget for). Higher = more conservative ghost rejection. ~2 means
   * we require the edge to survive a roughly 2σ adverse move over the exposure.
   */
  latencyRiskZ: number;
...
export function loadConfig(): { trading: TradingConfig; risk: RiskConfig } {
  ...
  return {
    trading: {
      ...
      executionLatencyMs: num('EXECUTION_LATENCY_MS', DEFAULT_TRADING.executionLatencyMs),
      volatilityPctPerSec: num('VOLATILITY_PCT_PER_SEC', DEFAULT_TRADING.volatilityPctPerSec),
      latencyRiskZ: num('LATENCY_RISK_Z', DEFAULT_TRADING.latencyRiskZ),
    },
    risk: {
      ...DEFAULT_RISK,
